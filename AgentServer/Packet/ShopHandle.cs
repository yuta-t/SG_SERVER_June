...
namespace AgentServer.Packet
{
    public class ShopHandle
    {
        public static void Handle_BuyItem(ClientConnection Client, PacketReader reader, byte last)
        {
            ...
            {
                bool isShuItem = info.ItemPosition == 50000;
                if (!isShuItem)
                {
                ...
                {
                    bool BuyCheckDone = BuyShuItemCheck(User, itemid, out var shuitemid);
                    if (BuyCheckDone)
                    {
                        string stritemid = string.Format("{0},", shuitemid);
                        ShuSystemHandle.Shu_GetUserItemInfo(User, stritemid, string.Empty, out var infos);
                        Client.SendAsync(new Shu_BuyItemOK(itemid, unk3, unk4, unk5, shuitemid, last));
                        Client.SendAsync(new MyroomGetAllItem_0X67_64(User, last));
                        Client.SendAsync(new Shu_BuyItemOK2(infos, last));
                    }
                }
            }
            else
            ...
            using (var con = new MySqlConnection(Conf.Connstr))
            {
                ...
                using (var cmd = new MySqlCommand(string.Empty, con))
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader(CommandBehavior.SingleRow))
                    {
                        ...
                        {
                            User.TR -= Convert.ToInt32(reader["itemPrice"]);
        ...
        private static bool BuyShuItemCheck(Account User, int itemnum, out long itemid)
        {
            itemid = 0;
            using (var con = new MySqlConnection(Conf.Connstr))
            {
                ...
                using (var cmd = new MySqlCommand(string.Empty, con))
                {
                    cmd.Parameters.Clear();
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandText = "usp_shu_buyItem";
                    cmd.Parameters.Add("usernum", MySqlDbType.Int32).Value = User.UserNum;
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            ...
                            User.TR -= Convert.ToInt32(reader["gameMoneyPrice"]);
                            User.Cash -= Convert.ToInt32(reader["CashPrice"]);
                            return true;
                        }
                    }
